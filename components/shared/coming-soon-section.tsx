...
import { Coffee, Handshake, Mail, ShoppingCart, Sparkles, UtensilsCrossed } from "lucide-react";
import type { Locale } from "@/lib/i18n/config";
import { Reveal } from "@/components/home/reveal";

export function ComingSoonSection({ locale, type = "restaurants" }: { locale: Locale; type?: "restaurants" | "cafes" | "supermarket" }) {
  const t = useTranslations("comingSoon");
  const Icon = type === "cafes" ? Coffee : type === "supermarket" ? ShoppingCart : UtensilsCrossed;
  const title = type === "cafes" ? t("cafesTitle") : type === "supermarket" ? t("supermarketTitle") : t("restaurantsTitle");
  const description =
    type === "cafes" ? t("cafesDescription") : type === "supermarket" ? t("supermarketDescription") : t("restaurantsDescription");
