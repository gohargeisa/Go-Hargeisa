      ...
      <div className="container-px mx-auto mt-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {cards.map((c, i) => (
            <div
              key={`${c.label}-${i}`}
              className="flex flex-col items-center gap-1.5 rounded-xl2 border border-ink/8 bg-white px-3 py-4 text-center shadow-soft transition-transform duration-200 hover:-translate-y-0.5 dark:border-white/10 dark:bg-white/[0.03]"
            >
              ...
              <p className="truncate text-sm font-bold text-ink dark:text-white">{c.value}</p>
            </div>
          ))}
        </div>
      </div>
