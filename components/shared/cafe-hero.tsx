...
import { Building2, Laptop, MapPin, Sparkles, Star, Wifi } from "lucide-react";
...
export function CafeHero({
  ...
  return (
    <section className="relative h-[42vh] max-h-[520px] min-h-[340px] w-full overflow-hidden sm:h-[52vh]">
      {hasRealImage ? (
        <Image src={image} alt={`${name} — cafe interior`} fill priority sizes="100vw" className="object-cover" />
      ) : (
        <div
        ...
        <div className="container-px absolute inset-x-0 bottom-0 mx-auto pb-6 text-white sm:pb-10">
          {(wifi || workingSpace) && (
            <div className="mb-3 flex flex-wrap gap-2">
              {wifi && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-xs font-semibold backdrop-blur-sm sm:text-sm">
                  <Wifi size={13} aria-hidden="true" /> Free WiFi
                </span>
              )}
              {workingSpace && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-xs font-semibold backdrop-blur-sm sm:text-sm">
                  <Laptop size={13} aria-hidden="true" /> Working Space
                </span>
              )}
            </div>
          )}
