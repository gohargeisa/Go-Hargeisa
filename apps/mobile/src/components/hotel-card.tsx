/**
 * `HotelCard` — mirrors `RestaurantCard`/`CafeCard`'s exact shape for the
 * separate hotels vertical.
 */
import { View } from "react-native";
import { Ionicons } from "@expo/vector-icons";
import { useRouter } from "expo-router";
import { useTranslation } from "react-i18next";
import type { HotelListItem } from "@gohargeisa/api";
...
export function HotelCard({ item }: { item: HotelListItem }) {
  ...
  return (
    <Card
      padded={false}
      onPress={() => router.push(`/hotels/${item.slug}`)}
