/**
 * Cafe detail — native, mirrors `restaurants/[slug].tsx` adapted to the
 * `CafeDetail` DTO (wifi/workingSpace badges, specialDrinks instead of a
 * cuisine tag row).
 */
...
import { useCafe } from "@/lib/queries";
...
export default function CafeDetailScreen() {
  ...
  const { data, isPending, isError, refetch } = useCafe(slug);
  const { itemCount: cartItemCount } = useCart();
  const myReview = useMyReview("cafe", data?.id);
  ...
  const badges = [
    data.wifi ? t("cafes.wifi", "Wi-Fi") : null,
    data.workingSpace ? t("cafes.workingSpace", "Workspace") : null,
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      >
        ...
        <View style={{ paddingHorizontal: spacing.screenX, paddingTop: 16, gap: 10 }}>
            ...
            <Button
              ...
              onPress={() =>
                router.push({
                  ...
                  params: { listingType: "cafe", id: data.id, name: data.name },
                })
              }
              variant="secondary"
              size="sm"
              fullWidth={false}
              icon={<Ionicons name="cafe-outline" size={16} color={theme.colors.primary} />}
            />
          ) : null}
          ...
          {data.specialDrinks.length > 0 ? (
            <View style={{ marginTop: 12, gap: 8 }}>
              <AppText variant="heading">{t("cafes.specialDrinks", "Special drinks")}</AppText>
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                {data.specialDrinks.map((d) => (
                  <View
                    key={d}
                    style={{
                  ...
                  >
                    <AppText variant="label">{d}</AppText>
                  </View>
                ))}
              </View>
            </View>
          ...
          <View style={{ marginTop: 16 }}>
            ...
            >
              {data.products.map((product) => (
                <ProductCard key={product.id} product={product} listingType="cafe" listingId={data.id} slug={data.slug} />
              ))}
            </ScrollView>
          </View>
        ) : null}
            ...
            <Pressable
              onPress={() =>
                router.push({
                  ...
                  params: { listingType: "cafe", id: data.id, slug: data.slug },
                })
              }
            >
              <AppText variant="label" color="primary">
